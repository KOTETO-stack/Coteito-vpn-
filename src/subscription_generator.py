...
def test_generate_clash_contains_trojan_node() -> None:
    ...
    assert proxy["password"] == "test-password"
...
def test_missing_credentials_are_not_published() -> None:
    node = make_node()
...
def test_write_subscription_creates_json_file(
    tmp_path: Path,
) -> None:
    node = make_node()
...
def test_generated_node_uses_display_name() -> None:
    node = make_node(name=None)

    result = generate_clash([node])

    assert result["proxies"][0]["name"] == (
        "Германия Берлин 🇩🇪"
    )